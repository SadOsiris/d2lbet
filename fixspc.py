...
def team_ana(scope):
    ...
    while (buf<len(content)):
        ...
        #var=t
        var=t.split('\t')
        if(int(var[6])!=scope and int(var[6])!=100-scope):
            buf+=1
            continue
        #print var[0],var[1],var[2],var[3],var[4],var[5],var[6],var[7],var[8],var[9]
        odds=var[8].split()
        if(len(odds)<7):
            lr=round(float(odds[0])-0.1)
            lu=round(float(odds[1])-0.1)
            lc=round(float(odds[2])-0.1)
            rr=round(float(odds[5])-0.1)
            ru=round(float(odds[4])-0.1)
            rc=round(float(odds[3])-0.1)
        else:
            lr=round(float(odds[1])-0.1)
            lu=round(float(odds[2])-0.1)
            lc=round(float(odds[3])-0.1)
            rr=round(float(odds[6])-0.1)
            ru=round(float(odds[5])-0.1)
            rc=round(float(odds[4])-0.1)
    ...
    team_draw=np.genfromtxt("./fixspec"+str(scope)+".txt",delimiter=' ',names=['foo','bar','br','bu','bc','nr','nu','nc'])
#    print(team_draw['bu'])
# Create the plot
...
# Save the figure in a separate file
    plt.savefig("fixspec"+str(scope)+".png")
