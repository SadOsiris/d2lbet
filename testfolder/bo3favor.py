...
#            nk+=4*rk
        ...
        buf+=1
        team_f=open("bo3favor"+".txt",'a')
        team_f.write(var[0]+" "+var[1]+" "+str(bv)+" "+str(nv)+'\n')
    team_draw=np.genfromtxt("./bo3favor"+".txt",delimiter=' ',names=['foo','bar','bv','nv'])
#    print(team_draw['bbku'])
# Create the plot
#    plt.plot(team_draw['bk'],label='bk',color='r')
    plt.plot(team_draw['bv'],label='bv',color='b')
