...
def team_ana(scope):
    ...
    nc=0.0
    buf=0
    team_f=open("favor"+str(scope)+".txt",'a')
    ...
    while (buf<len(content)):
        ...
        #var=t
        var=t.split('\t')
        if(int(var[6])>scope or int(var[6])<(100-scope)):
            buf+=1
            continue
    ...
    team_draw=np.genfromtxt("./favor"+str(scope)+".txt",delimiter=' ',names=['foo','bar','br','bu','bc','nr','nu','nc'])
#    print(team_draw['bu'])
# Create the plot
...
# Save the figure in a separate file
    plt.savefig("favor"+str(scope)+".png")
